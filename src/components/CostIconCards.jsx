  ...
  return (
    <div className="grid gap-3.5 sm:gap-4 md:grid-cols-2">
      {costCards.map(({ icon: Icon, title, text }, index) => (
        ...
        >
          <SpotlightCard borderGlow className={`min-h-36 p-3.5 sm:min-h-48 sm:p-5 md:min-h-52 lg:min-h-[10.75rem] lg:p-4 ${cardClassName}`}>
            <Icon
              aria-hidden="true"
              className="card-context-ghost-icon text-white"
              strokeWidth={1.65}
            />
            <div className="relative z-10 border-glow-card__icon mb-5 flex h-7 w-7 items-center justify-center rounded-full bg-white text-[#080709] sm:mb-10 sm:h-9 sm:w-9 lg:mb-6 lg:h-8 lg:w-8">
            ...
            <p className="relative z-10 mt-2 line-clamp-3 text-[0.76rem] leading-[1.18rem] text-white/48 sm:mt-3 sm:line-clamp-none sm:text-sm sm:leading-6 lg:mt-2 lg:text-[0.82rem] lg:leading-5">{text}</p>
          </SpotlightCard>
        </ScrollReveal>
      ))}
    </div>
